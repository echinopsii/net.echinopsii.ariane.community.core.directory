...
import net.echinopsii.ariane.core.directory.wat.json.ds.technical.system.OSTypeJSON;
import net.echinopsii.ariane.core.directory.wat.plugin.DirectoryJPAProviderConsumer;
import net.echinopsii.ariane.core.directory.wat.rest.organisational.CompanyEndpoint;
...
public class OSTypeEndpoint {
    ...
    public Response createOSType(@QueryParam("name")String name, @QueryParam("architecture")String arc) {
        if (name != null && arc != null) {
            ...
            if (subject.hasRole("ccsysadmin") || subject.isPermitted("ccDirComITiSysOst:create") ||
                subject.hasRole("Jedi") || subject.isPermitted("ccuniverse:zeone"))
            {
                em = DirectoryJPAProviderConsumer.getInstance().getDirectoryJpaProvider().createEM();
                OSType entity = findOSTypeByName(em, name);
    ...
    public Response deleteOSType(@QueryParam("id")Long id) {
        if (id!=0) {
            ...
            if (subject.hasRole("ccsysadmin") || subject.isPermitted("ccDirComITiSysOst:delete") ||
                subject.hasRole("Jedi") || subject.isPermitted("ccuniverse:zeone"))
            {
                em = DirectoryJPAProviderConsumer.getInstance().getDirectoryJpaProvider().createEM();
                OSType entity = findOSTypeById(em, id);
    ...
    public Response updateOSTypeName(@QueryParam("id")Long id, @QueryParam("name")String name) {
        if (id!=0 && name!=null) {
            ...
            if (subject.hasRole("ccsysadmin") || subject.isPermitted("ccDirComITiSysOst:update") ||
                subject.hasRole("Jedi") || subject.isPermitted("ccuniverse:zeone"))
            {
                em = DirectoryJPAProviderConsumer.getInstance().getDirectoryJpaProvider().createEM();
                OSType entity = findOSTypeById(em, id);
