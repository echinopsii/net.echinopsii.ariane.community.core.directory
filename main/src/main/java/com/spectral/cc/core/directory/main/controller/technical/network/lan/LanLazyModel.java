 ...
 * Directories Lan PrimeFaces Lazy Model
...
public class LanLazyModel extends LazyDataModel<Lan> {

    private static final Logger log = LoggerFactory.getLogger(LanLazyModel.class);

    private int              rowCount      ;
    private EntityManager    entityManager ;
    private List<Lan>        pageItems ;

    private Predicate[] getSearchPredicates(Root<Lan> root, Map<String,String> filters) {
    ...
    private void paginate(int first, String sortField, SortOrder sortOrder, Map<String,String> filters) {
        ...
        Root<Lan> root = countCriteria.from(Lan.class);
        ...
        TypedQuery<Lan> query = this.entityManager.createQuery(criteria);
    ...
    @Override
    public Lan getRowData(String rowKey) {
        for(Lan lan : pageItems) {
            if(lan.getId().equals(rowKey))
                return lan;
        }
        return null;
    }
    ...
    @Override
    public Object getRowKey(Lan lan) {
        return lan.getId();
    }

    public LanLazyModel setEntityManager(EntityManager entityManager) {
        this.entityManager = entityManager;
        return this;
    }

    @Override
    public List<Lan> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,String> filters) {
